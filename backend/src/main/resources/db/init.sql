-- ============================================================
-- 专利技术匹配系统 - 数据库初始化脚本（含全量表结构）
-- 数据库: MySQL 8.0+
-- 版本: 包含 V1(基础) + V2(收藏) + V3(LLM配置) + V4(用户配置选择)
-- ============================================================
...
VALUES ('admin', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt3GWoa', '管理员', 'admin', 1);
...
(
    ...
    source_type      VARCHAR(32)  DEFAULT 'FILE' COMMENT '来源类型：FILE-PDF上传/TEXT-文本录入/CSV-批量导入',
...
-- 插入系统默认离线配置（is_active=1 表示系统全局默认）
...
VALUES (0, 'Ollama 默认配置', 'offline', 'deepseek-r1:7b', 'qwen2.5:7b', 'bge-m3',
        ...
        '系统默认离线配置，使用 Ollama 三模型分工（对话/分析/向量）');

-- 插入系统默认在线配置：通义千问（未设为系统默认，需管理员启用）
...
VALUES (0, '通义千问（系统预设）', 'online',
        ...
        'text-embedding-v3', 0,
        '系统预设在线配置（阿里云 DashScope），需在环境变量中配置 ONLINE_API_KEY');

-- 插入系统默认在线配置：DeepSeek（未设为系统默认，需管理员启用）
INSERT INTO sys_llm_config (user_id, config_name, llm_mode, base_url, api_key, chat_model,
                            llm_model, embed_model, is_active, remark)
VALUES (0, 'DeepSeek（系统预设）', 'online',
        'https://api.deepseek.com', '', 'deepseek-chat', 'deepseek-chat',
        'bge-m3', 0,
        '系统预设在线配置（DeepSeek 官方），需在环境变量中配置 ONLINE_API_KEY');

-- ============================================================
-- 9. 用户 LLM 配置选择表（V4新增）
-- 记录每个用户当前选择的配置ID，彻底解耦系统配置与用户激活状态
-- 每个用户只有一条记录；未选择时回退系统 is_active=1 的配置
-- ============================================================
CREATE TABLE user_llm_selection
(
    user_id    BIGINT   NOT NULL COMMENT '用户ID（>0），主键确保每用户一条',
...
-- 完成
-- ============================================================
SELECT '数据库初始化完成！（含V1基础表 + V2收藏 + V3 LLM配置 + V4用户配置选择）' AS message;
