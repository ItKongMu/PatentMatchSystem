  ...
  <div class="page-container import-page">
    ...
    <div v-if="previewData" class="card preview-card">
      ...
      >
        ...
        <el-table-column prop="fullText" label="正文" width="140" align="center">
          <template #default="{ row }">
            <template v-if="row.fullText && row.fullText.length > 0">
              <!-- 超过 8 万字：danger 警告 -->
              <el-tooltip
                v-if="row.fullText.length >= 80000"
                placement="top"
                :show-after="300"
              >
                <template #content>
                  <div style="max-width:260px">
                    <strong>⚠️ 正文过长（{{ Math.round(row.fullText.length / 10000) }}万字）</strong><br/>
                    接近 10 万字上限，LLM 分析覆盖率有限，建议精简后再导入。<br/><br/>
                    预览：{{ truncateText(row.fullText, 120) }}
                  </div>
                </template>
                <el-tag type="danger" size="small" class="fulltext-tag">
                  <span>{{ Math.round(row.fullText.length / 1000) }}K字</span>
                  <span class="tag-warn-icon">⚠</span>
                </el-tag>
              </el-tooltip>
              <!-- 超过 5 万字：warning 提示 -->
              <el-tooltip
                v-else-if="row.fullText.length >= 50000"
                placement="top"
                :show-after="300"
              >
                <template #content>
                  <div style="max-width:260px">
                    <strong>💡 正文较长（{{ Math.round(row.fullText.length / 10000) }}万字）</strong><br/>
                    LLM 分析时将自动截取首尾内容，中间部分会被省略。<br/><br/>
                    预览：{{ truncateText(row.fullText, 120) }}
                  </div>
                </template>
                <el-tag type="warning" size="small" class="fulltext-tag">
                  <span>{{ Math.round(row.fullText.length / 1000) }}K字</span>
                  <span class="tag-warn-icon">!</span>
                </el-tag>
              </el-tooltip>
      ...
      <div class="import-options">
        <div class="auto-process-row">
          <el-checkbox v-model="autoProcess">
            <span class="auto-process-label">导入后自动处理</span>
          </el-checkbox>
          <el-tag v-if="autoProcess" type="success" size="small" effect="plain" class="recommend-tag">
            推荐
          </el-tag>
          <span class="auto-process-desc">（AI提取实体、向量化，让专利立即可搜索匹配）</span>
        </div>
        <div v-if="!autoProcess" class="manual-hint">
          <el-icon><WarningFilled /></el-icon>
          <span>未开启自动处理，导入后专利将处于「待处理」状态，需手动在专利列表中逐一触发处理</span>
        </div>
        <div v-if="autoProcess && previewData.validRows > 5" class="batch-hint">
          <el-icon><InfoFilled /></el-icon>
          <span>批量导入 {{ previewData.validRows }} 条，系统将自动排队处理，可在专利列表实时查看进度</span>
        </div>
      </div>
...
// 处理文件超出限制（on-exceed 的 files 参数为原生 File 对象数组）
const handleExceed = (files) => {
  uploadRef.value.clearFiles()
  const file = files[0]
  
  // 先验证新文件；若无效则同时清空旧的 selectedFile 和 previewData
  ...
  // genFileId() 为 File 对象附加 uid，使其符合 ElUpload 内部格式要求
  file.uid = genFileId()
...
const handleImport = async () => {
  ...
  // 提前获取 validData，避免弹框 confirm 期间 previewData 被其他操作修改
  ...
  try {
    ...
    // 提前保存 autoProcess 值：clearFile() 不会影响 autoProcess ref，
    // 但明确保存可让 goToList 的逻辑更清晰、健壮
    const shouldAutoProcess = autoProcess.value

    const res = await patentApi.importCsvPreviewData(validData, shouldAutoProcess)
    ...
    // 无论成功、失败还是用户取消，都必须重置 loading 状态
    importLoading.value = false
  }
}

// 跳转到专利列表
// importResult 中记录了是否有自动处理任务，通过检查 importedPatentIds 是否非空来决定是否轮询
const goToList = () => {
  showResultDialog.value = false
  const hasProcessingTasks = autoProcess.value && (importResult.value?.importedPatentIds?.length ?? 0) > 0
  router.push({
    path: '/patent/list',
    query: hasProcessingTasks ? { polling: 'true' } : undefined
  })
}
